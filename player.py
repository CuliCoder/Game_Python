...
class Game:
    ...
    def data(self):
        ...
        self.wall_image = pygame.image.load(
            path.join(image_folder, WALL_IMAGE)).convert()  # lấy hình tường
        self.bullet_image = pygame.image.load(
            path.join(image_folder, BULLET_IMAGE)).convert()  # lấy hình đạn
    ...
    def run(self):  # hàm này để chạy các chế độ game
        self.playing = True
        while self.playing:
            # tính thời gian trôi qua kể từ khung hình trước(giây)
    ...
    def draw(self):
        ...
        # Chỉnh kích thước hình ảnh nếu cần
            ...
            if not PLAYER:
                ...
                if current_time - death_time >= self.respawn_time:
                    ...
                    self.player1 = Player1(
                        self, pos_respawn_random[0], pos_respawn_random[1])
    ...
    def keys(self, button1, button2):  # hàm này dùng để demo thôi , khi ấn nút 1 thì mode 1v1 ,2 thì mode training, sau sẽ thay bằng hàm ấn các button trên UI để vào các chế độ
        # Thêm dòng này để sử dụng biến toàn cục
        global condition_mode_training, condition_mode_1v1
        mouse_pos = pygame.mouse.get_pos()
